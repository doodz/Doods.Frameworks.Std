...
namespace Doods.Framework.Ssh.Std.Requests.YetRequest
{
    ...
    public class IsRunningPidRequest : SshRequestBase
    {
        ...
        public IsRunningPidRequest(string pid): base($"ps -p {pid}")
        {
            ...
            _SshSerializer = new SshSerializer(new SshSerializerSettings() {Converters = new List<ISshConverter>() { new RunningPidRequestConverter(pid), new SshToSimpleStringConverter() } });
        }
        public IsRunningPidRequest(int pid) : base($"ps -p {pid}")
        {
           
            _SshSerializer = new SshSerializer(new SshSerializerSettings() { Converters = new List<ISshConverter>() { new RunningPidRequestConverter(pid.ToString()), new SshToSimpleStringConverter() } });
        }
    }
}