    ...
    public class NoHupRequest : SshRequestBase
    {
        ...
        private class NoHupRequestConverter : ISshConverter
        {
            ...
            public object Read(string reader, Type objectType)
            {
                ...
                var b = Int32.TryParse(tab?.Last().Trim(), out var result);
        ...
        public NoHupRequest(string cmd) : base($"nohup {cmd} >/dev/null 2>&1 </dev/null & echo {ReturnQuery.ResultPid} $! &")
        {
            this._SshSerializer = new SshSerializer(new SshSerializerSettings() { Converters = new List<ISshConverter>() { new NoHupRequestConverter(cmd), new SshToSimpleStringConverter()} });
        }
